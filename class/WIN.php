<?php /*********************************************************************
 *
 *   Copyright : (C) 2008 Nicolas Grekas. All rights reserved.
 *   Email     : p@tchwork.org
...
//	static function glob($f) {return glob($f);}
	...
	{
		return self::$FS->FolderExists(self::absPath($f)) ? new WIN_DIR($f) : dir($f);
	}

	static function closedir($d = null)
	{
		null === $d && $d = self::$DIR;
		return $d instanceof WIN_DIR ? $d->close() : closedir($d);
	}

	static function opendir($f, $context = null)
	{
		return self::$DIR = !$context && self::$FS->FolderExists(self::absPath($f)) ? new WIN_DIR($f) : opendir($f, $context);
	}

	static function readdir($d = null)
	{
		null === $d && $d = self::$DIR;
		return $d instanceof WIN_DIR ? $d->read() : readdir($d);
	}

	static function rewinddir($d = null)
	{
		null === $d && $d = self::$DIR;
		return $d instanceof WIN_DIR ? $d->rewind() : rewinddir($d);
	}
...
class WIN_DIR extends Directory
{
	public $path, $handle;
